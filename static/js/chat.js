...
async function loadMessages(userId) {
    try {
        ...
        messagesContainer.innerHTML = messages.map(message =>
            createMessageElement(message.text, message.to_user_id)  // Преобразуем каждое сообщение в HTML-элемент
...
async function sendMessage() {
    ...
    if (message && selectedUserId) {  // Проверяем, что сообщение не пустое и выбран собеседник
        const payload = {to_user_id: selectedUserId, text: message};  // Формируем данные для отправки
...
function addMessage(text, to_user_id) {
    const messagesContainer = document.getElementById('messages');
...
function createMessageElement(text, to_user_id) {
    const userID = parseInt(selectedUserId, 10);  // Преобразуем ID в число
    const messageClass = userID === to_user_id ? 'sent' : 'received';  // Определяем, кто отправил сообщение: мы или другой пользователь
